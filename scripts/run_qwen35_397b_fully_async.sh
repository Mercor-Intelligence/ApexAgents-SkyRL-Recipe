...
# ---------------------------------------------------------------------------
# Qwen3.5-397B-A17B fully-async 20-node run (flattened, single script).
#
# The production configuration from the blogpost, baked in as defaults:
# DPPO + prompt_mean (deltas 0.15), 160k context, 12 engines x TP=8 +
# 8 training nodes (Megatron TP=4 EP=16 CP=2 PP=4), MAX_CONCURRENCY=300,
# gpu_mem_util=0.93, S3 checkpointing via $S3_PATH.
# Every knob stays ${VAR:-default}-overridable.
#
# Notes for OSS SkyRL 0.3.0:
#   - generator.inference_engine.offload_kv_for_weight_sync landed upstream
#     AFTER 0.3.0 and is omitted here; restore it when bumping the skyrl pin.
#     It is what makes gpu_memory_utilization=0.93 safe during weight sync —
#     if sync OOMs without it, lower GPU_MEMORY_UTILIZATION to ~0.84.
# ---------------------------------------------------------------------------
...
# Scope NCCL_DEBUG=INFO to init-relevant subsystems so it doesn't flood logs.
export NCCL_DEBUG_SUBSYS=${NCCL_DEBUG_SUBSYS:-INIT,ALLOC,ENV}
...
export RUN_NAME=${RUN_NAME:-apex_qwen35_397b_tito_rl_dev1928}
MODAL_APP_NAME=${MODAL_APP_NAME:-apex_qwen35_397b}
...
# S3 checkpointing is a must at 397B (no NFS; per-node disks can't be re-merged).
CKPT_PATH="${CKPT_PATH:-$S3_PATH/$RUN_NAME/ckpt}"
export SKYRL_CHECKPOINT_TMPDIR=${SKYRL_CHECKPOINT_TMPDIR:-$SAVE_DIR/ckpt_staging}
mkdir -p "$TRIALS_DIR" "$EXPORT_PATH" "$SKYRL_CHECKPOINT_TMPDIR" 2>/dev/null || true
...
#-----------------
# vLLM parameters (0.93 relies on KV-offload-during-sync; see header)
#-----------------
...
#-----------------------
# Infrastructure: 20x8xH200 = 12 inference engines x TP=8 + 8 training nodes
# (Megatron TP=4, EP=16, CP=2, PP=4 => 64-way model parallel, DP=1)
#-----------------------
INFERENCE_TP="${INFERENCE_TP:-8}"
NUM_INFERENCE_ENGINES="${NUM_INFERENCE_ENGINES:-12}"
POLICY_NUM_GPUS_PER_NODE="${POLICY_NUM_GPUS_PER_NODE:-8}"
POLICY_NUM_NODES="${POLICY_NUM_NODES:-8}"
...
# Harness
# -------
LLM_MAX_PER_TURN_TOKENS="${LLM_MAX_PER_TURN_TOKENS:-40000}"
